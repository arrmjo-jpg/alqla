...
// مزوّد "Legacy" — نداء 365Scores مباشرة من Next.js (السلوك الأصلي قبل PlayerAggregateService).
// هذا الملف هو مصدر السبب الجذري لحادثة 2026-08-07 (كل نداء يخزَّن كملف fetch-cache منفصل بلا حد).
// يبقى موجوداً كخيار Feature Flag (`SPORT_PLAYER_PROVIDER=legacy`, الافتراضي حالياً) — راجع player.ts
// (المُنتقي) وplayer-aggregate.ts (البديل عبر PlayerAggregateService بالباك إند). لا تُعدَّل منطقياً هنا
// إلا لإصلاح خلل بنفس نمط 365 القديم — أي تحسين معماري يذهب لـplayer-aggregate.ts بدلاً منه.
//
...
export const legacyPlayerProvider: PlayerProvider = {
  getPlayer,
  getPlayerCareerData,
  getPlayerTrophies,
