...
const OwmWeatherSchema = z.object({
  ...
  sys: z.object({ sunrise: z.number().nullish(), sunset: z.number().nullish() }).nullish(),
});
...
const AR_TIME = new Intl.DateTimeFormat('ar', { hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'Asia/Amman' });
...
function toSnapshot(gov: Governorate, raw: OwmWeather): WeatherSnapshot {
  return {
    ...
    sunrise: raw.sys?.sunrise ? AR_TIME.format(new Date(raw.sys.sunrise * 1000)) : null,
    sunset: raw.sys?.sunset ? AR_TIME.format(new Date(raw.sys.sunset * 1000)) : null,
  };
}
