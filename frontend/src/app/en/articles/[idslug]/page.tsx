...
// English article page. Reuses the locale-aware article layer + SEO passthrough
// (PublicSeoBuilder → articleSeoToMetadata + JSON-LD) unchanged; English presentation only.
...
export async function generateMetadata({
  ...
  const article = await getArticle(idslug, 'en');
  if (!article) return { title: 'Article' };
  return articleSeoToMetadata(article, `${env.siteUrl}/en/articles/${idslug}`);
}

export default async function EnArticlePage({ params }: { params: Promise<{ idslug: string }> }) {
  const { idslug } = await params;
  const article = await getArticle(idslug, 'en');
  if (!article) notFound();

  // Canonical redirect to the article's real id-slug (English path).
  ...
  if (canonicalIdSlug && canonicalIdSlug.normalize('NFC') !== requested.normalize('NFC')) {
    permanentRedirect(`/en/articles/${canonicalIdSlug}`);
  }
        ...
        <header className="en-article-header">
          ...
          {article.primaryCategory && (
            <div style={{ marginTop: 16 }}>
              <Link href={enCategoryUrl(article.primaryCategory.slug)}>
