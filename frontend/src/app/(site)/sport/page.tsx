...
export default async function SportPage({ searchParams }: { searchParams: Promise<{ date?: string; live?: string }> }) {
  ...
  return (
    <>
      {/* إعلان الصفحات الداخليّة (منتجات) — أعلى الصفحة، يُخفى تلقائيًّا بلا إعلان ⇒ صفر مساحة. */}
      <AdZone
        zone="aalan_fy_alsfhat_aldakhlya_llmntjat"
        className="mx-auto flex w-full max-w-[1200px] justify-center px-4 pt-6 sm:px-6 lg:px-8"
      />
      <SportSection sport={DEFAULT_SPORT} date={date} today={today} live={sp.live === '1'} />
    </>
  );
}
