      ...
      <Container className="py-6">
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[340px_1fr]">
          <aside className="flex flex-col gap-6">
            ...
            {/* إعلان متعدّد — مكرّر (العمود الجانبيّ، بين المباريات والدول). */}
            <AdZone zone="aalan_fy_alryada_mtadd" className="flex justify-center" />
            <CountryAccordion countries={countries} />
          </aside>
          <main className="flex min-w-0 flex-col gap-6">
            <FeaturedSlider matches={featured} />
            {/* إعلان متعدّد (aalan_fy_alryada_mtadd) — مكرّر في القسم: هنا تحت الهيرو مباشرةً. */}
            <AdZone zone="aalan_fy_alryada_mtadd" className="flex justify-center" />
            {/* تحت الهيرو — نمط 365: ويدجت «الأهداف» عريض (يمين RTL، ⅔) + إعلان حقيقيّ (يسار، ⅓) قرب الدوريات */}
            {scorers.length > 0 && (
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-[2fr_1fr]">
                <TopScorers comps={scorers} />
                <AdZone zone="aalan_fy_qsm_alryada_kbyr" className="flex h-full items-center justify-center" />
              </div>
            )}
            <SportNews />
            {/* إعلان متعدّد — مكرّر (بعد الأخبار). */}
            <AdZone zone="aalan_fy_alryada_mtadd" className="flex justify-center" />
            ...
            {/* إعلان متعدّد — مكرّر (أسفل القوائم، نهاية العمود). */}
            <AdZone zone="aalan_fy_alryada_mtadd" className="flex justify-center" />
          </main>
        </div>
      </Container>
