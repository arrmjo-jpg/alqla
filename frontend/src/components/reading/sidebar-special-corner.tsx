...
// «زاوية خاصة» بالشريط الجانبيّ — نفس تصنيف #32 المستخدَم بالرئيسية (TwoColumnCategoryRow)، 5
// أخبار. غير موجود/فارغ ⇒ يُخفى.
...
export async function SidebarSpecialCorner({ locale = 'ar' }: { locale?: string } = {}) {
  ...
  const items = await getCategoryFeed(category.slug, 5, locale);
