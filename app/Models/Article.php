...
class Article extends Model
{
    ...
    // ─── Canonical URL foundation (ADR A3.6, revised 2026-07-23) ────────────

    /**
     * المسار القانوني: /article/{id} — بلا شرطة مائلة زائدة، بلا تاريخ، بلا
     * slug، بلا بادئة لغة. المعرّف وحده لأنه الهوية الثابتة الوحيدة للمقال؛
     * slug يبقى زخرفياً قابلاً للتعديل بحرّية دون أن يكسر أي رابط قائم — أساس
     * حلّ مشكلة تضارب الكاش (كل مسار يُحسَب من id، لا من أي نص وارد في الرابط).
     *
     * (يُلغي الشكل السابق المعتمد على التاريخ /news/{dd}/{mm}/{yyyy}/{id}: كان
     * يجعل المسار يتغيّر مع published_at بلا داعٍ؛ id وحده أبسط ويطابق الواجهة
     * الفعليّة اليوم. لم يُنشَر هذا الشكل السابق علناً قط، فلا حاجة لترحيل بيانات).
     *
     * أي طلب لمسار قديم (/articles/{id}(-{slug})? أو الشكل القديم المعتمد على
     * التاريخ) يُحوَّل 301 عبر ArticleRedirectResolver/article_url_history —
     * سجلّ استيراد ووردبريس (ImportWpPostAction) يبقى المصدر الحقيقيّ الوحيد
     * لروابط خارجية قديمة فعلاً مفهرسة، ويُحلّ حيّاً إلى قيمة canonicalPath()
     * الحاليّة أياً كانت — لا حاجة لأي ترحيل بيانات عند تغيّر هذه الدالة.
     */
